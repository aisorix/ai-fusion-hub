...
export default function AdminAILive() {
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-sm text-slate-500">Most recent 100 AI calls across the platform.</p>

      <Card className="bg-white border-slate-200 divide-y divide-slate-200">
        {events.length === 0 && <div className="p-8 text-center text-slate-500">Waiting for events…</div>}
        {events.map((e) => (
          <div key={e.id} className="p-3 flex items-center gap-3 text-sm hover:bg-slate-50">
            ...
            <span className="text-slate-500 w-32 truncate">{e.model ?? "—"}</span>
            <span className="text-slate-500 tabular-nums w-20 text-right">{(e.tokens_in ?? 0) + (e.tokens_out ?? 0)} tok</span>
            <span className="text-slate-500 tabular-nums w-20 text-right">{e.latency_ms ?? "—"}ms</span>
