  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div><h1 className="text-2xl font-bold">Coupons</h1><p className="text-sm text-slate-400">Promotional discount codes.</p></div>
            ...
            <TableBody>
              {rows.map((c) => (
                <TableRow key={c.id} className="border-slate-800">
                  ...
                  <TableCell className="text-xs text-slate-400">{c.expires_at ? new Date(c.expires_at).toLocaleDateString() : "Never"}</TableCell>
      ...
      <Dialog open={creating} onOpenChange={setCreating}>
        <DialogContent className="bg-slate-900 border-slate-800 text-slate-100">
          ...
          <div className="space-y-3">
            <Input placeholder="CODE (e.g. LAUNCH20)" value={form.code} onChange={(e) => setForm({ ...form, code: e.target.value.toUpperCase() })} className="bg-slate-800 border-slate-700" />
            <Input placeholder="Description" value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} className="bg-slate-800 border-slate-700" />
            <div className="grid grid-cols-2 gap-3">
              <Input type="number" placeholder="Percent off (1-100)" value={form.percent_off} onChange={(e) => setForm({ ...form, percent_off: e.target.value })} className="bg-slate-800 border-slate-700" />
              <Input type="number" step="0.01" placeholder="Amount off" value={form.amount_off} onChange={(e) => setForm({ ...form, amount_off: e.target.value })} className="bg-slate-800 border-slate-700" />
            </div>
            <Input type="number" placeholder="Max redemptions (optional)" value={form.max_redemptions} onChange={(e) => setForm({ ...form, max_redemptions: e.target.value })} className="bg-slate-800 border-slate-700" />
            <Input type="datetime-local" placeholder="Expires" value={form.expires_at} onChange={(e) => setForm({ ...form, expires_at: e.target.value })} className="bg-slate-800 border-slate-700" />
          </div>
          <DialogFooter>
