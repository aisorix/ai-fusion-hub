...
const Features = () => {
  ...
  return (
    <section id="Features" className="py-10 sm:py-16 md:py-24 bg-muted/30 relative overflow-hidden">
      {/* Background Pattern */}
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 md:px-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5 lg:gap-6 mb-8 sm:mb-14">
          {premiumFeatures.map((feature, index) => {
            const IconComponent = feature.icon;
            return (
              <div
                key={index}
                className={`relative group bg-card rounded-xl sm:rounded-2xl p-5 sm:p-6 lg:p-8 border border-border overflow-hidden transition-all duration-500 ${
                  feature.comingSoon 
                    ? 'opacity-70' 
              ...
              >
                ...
                {feature.comingSoon && (
                  <div className="absolute top-2 sm:top-3 right-2 sm:right-3 z-10">
                    <span className="px-2 sm:px-3 py-0.5 sm:py-1 bg-foreground text-background text-[10px] sm:text-xs font-bold rounded-full">
                      {t('comingSoon')}
                    </span>
                  </div>
                )}
                ...
                <div className="relative z-10">
                  <div className={`inline-flex p-2.5 sm:p-3 rounded-xl bg-gradient-to-br ${feature.gradient} text-primary-foreground shadow-lg mb-3 sm:mb-4 group-hover:scale-110 transition-transform duration-300`}>
                  ...
                  {!feature.comingSoon && (
                    <div className="mt-3 sm:mt-4 flex items-center gap-2 text-primary font-semibold opacity-0 group-hover:opacity-100 transition-opacity text-xs sm:text-sm">
                      {language === 'en' ? 'Learn more' : 'আরও জানুন'} <ArrowRight className="w-3.5 h-3.5 sm:w-4 sm:h-4" />
                    </div>
                  )}
                </div>
              </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5">
          {regularFeatures.map((feature, index) => {
            ...
            return (
              <div
                key={index}
                className="group bg-card rounded-xl sm:rounded-2xl p-4 sm:p-5 lg:p-6 border border-border hover:border-primary/20 hover:shadow-xl transition-all duration-300 hover:-translate-y-1"
              >
                <div className={`inline-flex p-2.5 sm:p-3 rounded-xl bg-gradient-to-br ${feature.gradient} text-primary-foreground shadow-md mb-3 sm:mb-4 group-hover:scale-110 transition-transform duration-300`}>
