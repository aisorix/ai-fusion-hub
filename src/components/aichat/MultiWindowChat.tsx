  ...
  const handleSend = useCallback(
    async (content: string, attachments: Attachment[]) => {
      ...
      const sendToWindow = async (window: ChatWindow) => {
        ...
        const systemPrompt =
          "You are Sorix AI, a helpful, intelligent assistant. Be concise but thorough in your responses. Use markdown formatting when appropriate.";
        ...
        console.log(`[${modelName}] Sending message, estimated input tokens: ${inputTokens}${hasAttachments ? ' (server runs 2-stage: gpt-5.4-nano analyzer → ' + modelName + ')' : ''}`);
    ...
    <div className="flex-1 flex flex-col h-full bg-background">
      ...
      <div className="flex-1 overflow-hidden p-2 sm:p-4">
        ...
        >
          {chatWindows.map((window) => (
            <ChatWindowPanel key={window.id} window={window} canClose={chatWindows.length > 1} />
          ))}
        </div>
      </div>
      ...
      <div className="pb-4 sm:pb-6">
        <SharedChatInput
          ...
          userPlan={user.plan}
        />
        <p className="text-[10px] sm:text-xs text-center mt-2 text-muted-foreground"></p>
      </div>
...
const ChatWindowPanel = ({ window, canClose }: ChatWindowPanelProps) => {
  ...
  return (
    <div className={cn("flex flex-col rounded-xl sm:rounded-2xl border overflow-hidden", "bg-card border-border")}>
      {/* Window Header */}
        ...
        {window.messages.length === 0 ? (
          ...
          <div className="divide-y divide-border/30">
            {window.messages.map((message, msgIndex) => {
              ...
              const showActions =
                showActionsFor === message.id && isAssistant && message.content && !window.isStreaming;

              return (
                <div
                  key={message.id}
                  className={cn("py-3 sm:py-4 px-3 sm:px-4 group", isAssistant && "bg-muted/20")}
                            ...
                            >
                              ...
                              {(messageReactions[message.id]?.size || 0) > 0 && (
                                <div className="flex items-center gap-0.5 px-1.5 py-0.5 rounded-full bg-muted/50">
                                  {EMOJI_REACTIONS.filter((r) => messageReactions[message.id]?.has(r.key)).map(
                                    (reaction) => (
                                      <span key={reaction.key} className="text-xs">
                                        {reaction.emoji}
                                      </span>
                                    ),
                                  )}
