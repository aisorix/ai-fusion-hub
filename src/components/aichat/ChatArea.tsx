...
const ChatArea = ({ onOpenVoiceMode }: ChatAreaProps) => {
  ...
  return (
    // FIX: Changed 'h-full' to 'h-[100dvh]' to fix mobile viewport height issues
    // Added 'relative' to establish stacking context
    <div className="flex-1 flex flex-col h-[100dvh] min-w-0 bg-background relative">
      {/* Header with Model Selector - Desktop only (mobile has MobileHeader) */}
      {/* Added shrink-0 so header doesn't shrink when keyboard opens */}
      ...
      {/* Main Content */}
      {/* Added 'relative' for better positioning context */}
      ...
      {/* Added z-20 to ensure input stays above message list content */}
      <div className="pb-2 sm:pb-3 md:pb-4 shrink-0 bg-background z-20 relative">
