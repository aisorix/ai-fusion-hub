...
import CineshootModelSelector from './CineshootModelSelector';
import type { CineshootModel } from './cineshootModels';

interface Props {
  onGenerate: (prompt: string, attachments?: Attachment[]) => void;
  isGenerating: boolean;
  disabled?: boolean;
  selectedModel: CineshootModel;
...
const formatSize = (b: number) => b < 1024 * 1024 ? `${(b / 1024).toFixed(0)} KB` : `${(b / (1024 * 1024)).toFixed(0)} MB`;

const CineshootPromptBar: React.FC<Props> = ({
  onGenerate, isGenerating, disabled, selectedModel, onSelectModel, userPlan, onUpgrade,
  injectPrompt, injectAttachmentUrl, injectKey,
}) => {
  ...
  useAutoFocusInput(textareaRef, [attachments.length, isParsing, showAttachMenu, showToolsMenu], !!disabled || isGenerating, true);

  useEffect(() => {
    if (injectKey === undefined) return;
    if (typeof injectPrompt === 'string') setPrompt(injectPrompt);
    if (injectAttachmentUrl) {
      setAttachments(prev => [...prev, { type: 'image', url: injectAttachmentUrl, name: 'template-ref.png', size: 0, fileType: 'image' }]);
    }
    ...
    for (const file of files) {
      ...
      try {
        if (file.type.startsWith('image/')) {
          ...
          setAttachments(prev => [...prev, { type: 'image', url, name: file.name, size: file.size, fileType: 'image' }]);
        } else {
  ...
  const handleSubmit = () => {
    ...
    onGenerate(prompt.trim(), attachments.length ? attachments : undefined);
    setAttachments([]);
  };

  const handleKeyDown = (e: React.KeyboardEvent) => {
    if (isSubmitEnter(e)) { e.preventDefault(); handleSubmit(); }
  };
  ...
  return (
    <div className="w-full">
      ...
      <div className={cn(
        ...
        'bg-muted/40 border-border/50 focus-within:border-primary/40 focus-within:bg-muted/60',
        ...
        <div className="flex items-center justify-between gap-1 mt-1">
                  ...
                  >
                    <div className="px-4 py-2 border-b border-border bg-muted/50">
                      <div className="flex items-center justify-between gap-2">
                        ...
                        <span className="text-xs font-medium px-1.5 py-0.5 rounded bg-muted text-muted-foreground">{formatSize(sizeLimit)}</span>
                      </div>
                    </div>
                    ...
                    <button onClick={() => videoInputRef.current?.click()} className="flex items-center gap-3 px-4 py-3 w-full hover:bg-accent transition-colors">
                      <div className="w-8 h-8 rounded-lg bg-rose-500/10 flex items-center justify-center"><Paperclip className="w-4 h-4 text-rose-500" /></div>
                      <span className="text-sm">Video to Video</span>
                    </button>
                  </motion.div>
                )}
              </AnimatePresence>
            ...
            <div className="relative">
              <button
                ...
                className={cn(
                  ...
                  showToolsMenu ? 'bg-background text-foreground border-border' : 'border-border/60 text-muted-foreground hover:text-foreground hover:bg-background'
                )}
              >
          ...
          <div className="flex items-center gap-1.5 shrink-0 max-w-[55%] sm:max-w-none">
            ...
            <VoiceDictationButton
              onTranscript={t => setPrompt((p: string) => (p ? p.trimEnd() + ' ' + t : t))}
              disabled={isGenerating || disabled}
            ...
            <button
              ...
              className={cn(
                ...
                hasContent ? 'bg-foreground text-background hover:opacity-90' : 'bg-muted text-muted-foreground opacity-50',
...
export default CineshootPromptBar;
